...
void lerComando(char ***ptab, int *l, int *c, StackMovimentos *stack)
{
    ...
    while (1) {
        ...
        /*──────── desfazer ───*/
            ...
            // Desfazer todas as alterações até encontrar o marcador
            while (!(m.x == -1 && m.y == -1)) {
                (*ptab)[m.y][m.x] = m.valorAntigo;
                if (!pop(stack, &m))
                    break; // evitar loop infinito se stack vazia
            }
        }
